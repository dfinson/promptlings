...
| Phase sequence, gates, rollback, resume, remediation, and `[P]` dispatch | Fleet through `/speckit.fleet.run` | Invoke Fleet and preserve its prompts |
| Fleet's cross-model review | Fleet through `/speckit.fleet.review` | Receive its report as a required aggregate input |
...
13. Never recreate or manually sequence Fleet phases, gates, rollback, resume detection, remediation loops, test-runner detection, or `[P]` dispatch; invoke `/speckit.fleet.run`.
...
22. Never count `NOT_APPLICABLE` in aggregate verdict calculation; state the scope mismatch plainly.
...
2. Run read-only setup preflight:
   ...
   6. In an initialized project, run `specify integration list`, `specify extension list`, and `specify extension info fleet` when supported.
   7. Read `.specify/extensions/fleet/fleet-config.yml` when present.
   8. Inspect current Fleet artifacts without modification.
...
7. For project initialization:
   ...
   3. Report directories written, overwrite behavior, and repository effects.
   4. Propose the exact `specify init --here --integration <key> --script <variant>` command.
...
10. Handle Fleet configuration through its documented surfaces:
    ...
    3. For persistent changes to `models.review`, `parallel.max_concurrency`, `verify.auto_prompt_install`, or `verify.install_url`, name the exact key and `.specify/extensions/fleet/fleet-config.yml` path.
...
15. When setup is `READY`, select the Fleet invocation:
    1. Use `/speckit.fleet.run <feature description>` for a new request.
    2. Use `/speckit.fleet.run` for artifact-based resume.
    3. Pass a phase override through the same command after an explicit user choice.
...
19. Invoke Fleet and let Fleet own sequencing, resume, gates, rollback, remediation, and parallel work.
...
30. Preserve `NOT_APPLICABLE` with its scope note and omit it from aggregate verdict calculation.
...
33. Compute aggregate verdict:
    ...
    3. `APPROVE` otherwise.
34. Red-team roster completeness, evidence validity, finding preservation, verdict math, and verbatim Fleet prompts. Route new concerns through the complete reviewer procedure.
35. Output the Approval Template at every setup or Fleet gate.
36. Recommend Fleet Revise or Rollback for `REQUEST_CHANGES`, and completion of missing reviews for `REVIEW_INCOMPLETE`.
37. Forward the user's Fleet choice and relevant finding tasks to the owning phase.
38. Repeat from step 1 on every new turn.
39. Before completion, verify setup identity, Fleet terminal result, current triage, Fleet review, complete roster accounting, valid reviewer results, finding preservation, aggregate math, unresolved verdicts, warnings, and user overrides.
40. Output the Completion Template. Use `Outcome: INCOMPLETE` when a required condition remains unmet and name the exact resumption command or artifact.
...
Fleet invocation: </speckit.fleet.run form used>
...
Aggregate verdict: <APPROVE | REQUEST_CHANGES | REVIEW_INCOMPLETE>
...
Resume: <exact official setup command, /speckit.fleet.run, Fleet prompt, or artifact point>
```
