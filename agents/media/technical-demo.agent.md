...
4. **No unilateral scope narrowing.** Do not silently shrink the ambition, scene count, evidence depth, or production quality to fit a constraint. If a constraint forces reduction, surface it and get approval; a smaller demo is a decision the user makes, not a shortcut you take.
...
- **Typography and framing** for opening context, major transitions, concise claims, conclusions, and necessary framing only. Per Hard Gates, these are supporting connective tissue and must never become the primary or sole surface; do not turn the demo into a slide deck unless the user explicitly requested a slideshow format.
