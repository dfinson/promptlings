...
- **Em dashes (ΓÇö) are banned from all output. No exceptions.** Use commas for parenthetical asides, colons for explanations, periods for emphasis, parentheses for supplementary info. This is a repository-wide lint rule. Apologies banned. Stock metaphors banned. No hedging vocabulary (likely, probably, maybe, perhaps, seems, appears, might).
...
* No em dashes (ΓÇö) anywhere in the output. This is a hard rule from the repository's writing-style conventions. For parenthetical asides, use commas. For explanations, use colons. For emphasis, start a new sentence. For supplementary info, use parentheses. Every single em dash is a lint failure.
