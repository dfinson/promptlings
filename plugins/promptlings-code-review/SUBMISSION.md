...
1. **The name is effectively immutable once someone else's catalog owns it.** The official directory README is explicit that a published plugin's `name` should not change, because users have it installed under that slug. It also documents an escape hatch: a `renames` map at the top of the marketplace file migrates existing installs, and the official catalog carries live entries using it. That hatch is available in `.claude-plugin/marketplace.json` here, since this repository owns that file. It is not available once an entry lands in a catalog controlled by someone else. Treat `promptlings-code-review` as the commitment for any submitted entry, and use `displayName` for cosmetic relabeling.
...
5. **Example outputs.** Unedited runs of both agents against a real pull request are committed in `examples/`. Nothing in the stated criteria requires them, but they let a reviewer judge output quality without running the agent.
